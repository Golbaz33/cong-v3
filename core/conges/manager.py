...
# Version finale intégrant toutes les améliorations, fonctionnalités et corrections de bugs.
...
class CongeManager:
    """
    Le "cerveau" de l'application. Gère toute la logique métier.
    Ne communique JAMAIS directement avec l'interface utilisateur.
    Sert de façade unique pour l'accès à la base de données.
    """
    ...
    # =========================================================================
    # PASSERELLES POUR L'ACCÈS AUX DONNÉES (AXE 2)
    # =========================================================================
    ...
    # =========================================================================
    # LOGIQUE MÉTIER PURE (AXE 1)
    # =========================================================================

    def save_agent(self, agent_data, is_modification=False):
        if is_modification:
            return self.db.modifier_agent(agent_data['id'], agent_data['nom'], agent_data['prenom'], agent_data['ppr'], agent_data['grade'], agent_data['solde'])
        else:
            return self.db.ajouter_agent(agent_data['nom'], agent_data['prenom'], agent_data['ppr'], agent_data['grade'], agent_data['solde'])
    ...
    def delete_conge(self, conge_id):
        ...
        try:
            if conge.statut == 'Annulé':
                ...
                self.db.execute_query("DELETE FROM conges WHERE id=?", (conge_id,)); return True
            else:
    ...
    def handle_conge_submission(self, form_data, is_modification):
        try:
            ...
            if overlaps:
                ...
                if form_data['type_conge'] == 'Congé annuel' or len(annual_overlaps) != len(overlaps):
                    raise ValueError("Chevauchement invalide. Vous ne pouvez remplacer que des congés annuels.")
        ...
        except (ValueError, sqlite3.Error) as e: raise e
        except Exception as e: logging.error(f"Erreur soumission congé: {e}", exc_info=True); raise e
    ...
    # =========================================================================
    # MÉTHODES INTERNES (NON APPELÉES PAR L'UI)
    # =========================================================================
        ...
        try:
            ...
            if parent_conge_row:
                ...
                if parent_conge.type_conge in CONFIG['conges']['types_decompte_solde']:
                    # CORRECTION DU BUG DE LA FAUTE DE FRAPPE
    ...
    def split_or_replace_leaves(self, annual_overlaps, form_data):
        ...
        try:
            self.db.conn.execute('BEGIN TRANSACTION')
            cursor = self.db.conn.cursor()
            new_start = validate_date(form_data['date_debut']); new_end = validate_date(form_data['date_fin'])
            ...
            for conge in annual_overlaps:
                ...
                if conge.date_debut < new_start:
                    self._creer_segment(cursor, conge.agent_id, conge.date_debut, new_start - timedelta(days=1), holidays_set)
                if conge.date_fin > new_end:
                    self._creer_segment(cursor, conge.agent_id, new_end + timedelta(days=1), conge.date_fin, holidays_set)
    ...
    def _handle_certificat_save(self, form_data, is_modification, conge_id):
        ...
        if os.path.exists(new_path) and new_path != original_path:
            try:
                ...
                self.db.execute_query("REPLACE INTO certificats_medicaux (conge_id, duree_jours, chemin_fichier) VALUES (?, ?, ?)", (conge_id, cert_model.duree_jours, cert_model.chemin_fichier))
