...
# Version finale avec correction du threading pour SQLite.

import tkinter as tk
from tkinter import ttk, messagebox, filedialog # On ajoute filedialog ici
...
# ... (les fonctions format_date_for_display_short et treeview_sort_column ne changent pas)
def format_date_for_display_short(date_obj):
    if not date_obj: return ""
    try:
        if hasattr(date_obj, 'strftime'): return date_obj.strftime("%d/%m/%y")
        return parser.parse(str(date_obj)).strftime("%d/%m/%y")
    except (ValueError, TypeError): return str(date_obj)
...
class MainWindow(tk.Tk):
    # ... (__init__, on_close, set_status, create_widgets ne changent pas)
    ...
    def create_widgets(self):
        ...
        self.text_stats = tk.Text(stats_frame, wrap=tk.WORD, font=('Courier New', 10), height=8, relief=tk.FLAT, background=self.cget('bg')); self.text_stats.pack(fill=tk.BOTH, expand=True, padx=10, pady=5); self.text_stats.config(state=tk.DISABLED)
    ...
    # ... (les fonctions get_selected_agent_id, get_selected_conge_id, add_agent_ui, etc. jusqu'à on_conge_double_click ne changent pas)
    ...
    def delete_selected_conge(self):
        ...
        try:
            ...
            msg = "Êtes-vous sûr de vouloir supprimer définitivement ce congé annulé de l'historique ?" if conge.statut == 'Annulé' else "Êtes-vous sûr de vouloir supprimer ce congé ?\nS'il fait partie d'une division, le congé d'origine sera restauré."
    ...
    def refresh_agents_list(self, agent_to_select_id=None):
        ...
        self.prev_button.config(state="normal" if self.current_page > 1 else "disabled"); self.next_button.config(state="normal" if self.current_page < self.total_pages else "disabled")
    ...
    def refresh_conges_list(self, agent_id):
        ...
        for annee in sorted(conges_par_annee.keys(), reverse=True):
            ...
            for conge in sorted(conges_par_annee[annee], key=lambda c: c.date_debut):
                cert_status = "✅ Justifié" if self.manager.get_certificat_for_conge(conge.id) else "❌ Manquant" if conge.type_conge == 'Congé de maladie' else ""
                interim_info = ""
                if conge.interim_id:
    ...
    def refresh_stats(self):
        ...
        try:
            all_conges = self.manager.get_all_conges(); nb_agents = self.manager.get_agents_count()
    ...
    # --- AXE 3 : GESTION DES TÂCHES LONGUES (IMPORT/EXPORT) ---
    
    def export_agents(self):
        # CHANGEMENT : On demande le nom du fichier de destination dans le thread principal.
        save_path = filedialog.asksaveasfilename(
            defaultextension=".xlsx", filetypes=[("Fichiers Excel", "*.xlsx")],
            title="Exporter la liste des agents",
            initialfile=f"Export_Agents_{datetime.now().strftime('%Y-%m-%d')}.xlsx"
        )
        if not save_path: return # Annulé
        
        db_path = self.manager.db.db_file
        self._run_long_task(
            task_lambda=lambda: export_agents_to_excel(db_path, save_path),
            on_complete=self._on_task_complete,
            status_message="Exportation des agents en cours..."
        )
    ...
    def export_conges(self):
        ...
        db_path = self.manager.db.db_file
        self._run_long_task(
            task_lambda=lambda: export_all_conges_to_excel(db_path, save_path),
            on_complete=self._on_task_complete,
            status_message="Exportation de tous les congés en cours..."
        )

    def import_agents(self):
        # CHANGEMENT : On demande le fichier source dans le thread principal.
        source_path = filedialog.askopenfilename(
            title="Sélectionner un fichier Excel à importer",
            filetypes=[("Fichiers Excel", "*.xlsx")]
        )
        if not source_path: return # Annulé

        db_path = self.manager.db.db_file
        self._run_long_task(
            task_lambda=lambda: import_agents_from_excel(db_path, source_path),
            on_complete=self._on_import_complete,
            status_message="Importation des agents depuis Excel en cours..."
        )

    # Le reste des fonctions de gestion de thread ne change pas.
    def _run_long_task(self, task_lambda, on_complete, status_message):
        self.set_status(status_message); self.config(cursor="watch"); self._toggle_buttons_state("disabled")
        result_container = []
        def task_wrapper():
    ...
    def _check_thread_completion(self, thread, result_container, on_complete):
        if thread.is_alive():
            ...
            on_complete(result)
            self.config(cursor=""); self._toggle_buttons_state("normal"); self.set_status("Prêt.")
    def _on_task_complete(self, result):
        if isinstance(result, Exception):
            # Le message d'erreur contient maintenant le détail de l'exception levée par le thread.
